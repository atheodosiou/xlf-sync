...
describe("XLIFF 1.2 Serialization", () => {
	it("should serialize a simple XLIFF 1.2 file", () => {
		const entries = new Map<string, MessageEntry>([
			[
				"welcome",
				{ key: "welcome", sourceXml: "Welcome", targetXml: "Willkommen" },
			],
		]);
		...
		const xml = writeV12(parsed.raw, entries, [], {
			newTarget: "todo",
			obsolete: "mark",
		});
	...
	it("should mark obsolete entries with comment", () => {
		const entries = new Map<string, MessageEntry>([
			[
				"current",
				{ key: "current", sourceXml: "Current", targetXml: "Aktuell" },
			],
		]);
		...
		const obsolete = new Map<string, MessageEntry>([
			[
				"deprecated",
				{ key: "deprecated", sourceXml: "Deprecated", targetXml: "Veraltet" },
			],
		]);
		...
		const xml = writeV12(parsed.raw, entries, Array.from(obsolete.keys()), {
			newTarget: "todo",
			obsolete: "mark",
		});
  ...
  <file source-language="en" target-language="es" datatype="plaintext">
    <body>
      <trans-unit id="hello">
		...
		const serialized = writeV12(parsed.raw, parsed.entries, [], {
			newTarget: "todo",
			obsolete: "mark",
		});
