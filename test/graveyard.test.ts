...
import type { ParsedXlf } from "../src/types/model.js";

describe("Graveyard Logic", () => {
	it("should return empty map for no obsolete keys", () => {
		const parsed = { version: "1.2" } as unknown as ParsedXlf;
	...
	it("should build graveyard entries for XLIFF 2.0", () => {
		const parsed = {
			...
			raw: {
				xliff: {
					file: {
						unit: [{ "@_id": "k1", segment: { source: "S1", target: "T1" } }],
					},
				},
			},
		} as unknown as ParsedXlf;
