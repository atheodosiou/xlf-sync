      ...
      <trans-unit id="testId" approved="yes">
        ...
        <context-group purpose="location">
          ...
          <context context-type="linenumber">10</context>
			...
			const syncResult = syncLocale(parsed.entries, new Map(), {
				newTarget: "todo",
				obsolete: "mark",
			});
			const output = writeV12(parsed.raw, syncResult.merged, [], {
				newTarget: "todo",
				obsolete: "mark",
			});
			...
			expect(output).toContain(
				'<note from="description" priority="1">Greeting</note>',
			);
			expect(output).toContain(
				'<context context-type="sourcefile">app/app.component.ts</context>',
			);
...
<xliff version="1.2">
  <file><body><trans-unit id="1" approved="yes"><source>Hi</source><target>Hola</target><note>Locale Note</note></trans-unit></body></file>
			...
			const res = syncLocale(source.entries, locale.entries, {
				newTarget: "todo",
				obsolete: "mark",
			});
			const mergedEntry = res.merged.get("1");
			expect(mergedEntry).toBeDefined();
			if (!mergedEntry) throw new Error("Entry not found");
    ...
    <body>
      <trans-unit id="old1" approved="yes">
        ...
        <note from="me">Keep this</note>
			...
			const parsed = parseV12(parser.parse(xml));
			const res = syncLocale(new Map(), parsed.entries, {
				newTarget: "todo",
				obsolete: "mark",
			});
			const output = writeV12(parsed.raw, res.merged, res.obsoleteKeys, {
				newTarget: "todo",
				obsolete: "mark",
			});
    ...
    <unit id="u1" translate="no">
      ...
      <segment>
        <source>Hello</source>
			...
			const output = writeV20(parsed.raw, syncResult.merged, [], {
				newTarget: "todo",
				obsolete: "mark",
			});
 ...
 <file>
   <unit id="u1" translate="no">
     ...
     <segment><source>Old</source><target>Old Trg</target></segment>
			...
			const output = writeV20(parsed.raw, res.merged, res.obsoleteKeys, {
				newTarget: "todo",
				obsolete: "mark",
			});
