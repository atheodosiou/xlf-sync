...
import type { MessageEntry, ParsedXlf } from "../src/types/model.js";
...
describe("Dashboard Command", () => {
	...
	it("should successfully generate a dashboard", async () => {
		const mockFiles = {
			localeFiles: [{ locale: "el", filePath: "messages.el.xlf" }],
		...
		vi.mocked(parseXlf).mockReturnValue({
			version: "1.2",
			entries: new Map([["k1", { key: "k1", sourceXml: "", targetXml: "T" } as MessageEntry]]),
		} as unknown as ParsedXlf);
		vi.mocked(isUntranslated).mockReturnValue(false);
		vi.mocked(discover.discoverFiles).mockResolvedValue(
			mockFiles as unknown as { localeFiles: { locale: string; filePath: string }[]; sourcePath: string },
		);
		...
		vi.mocked(dashboardGen.generateDashboardHtml).mockReturnValue(
			"<html></html>",
		);
	...
	it("should handle failures gracefully", async () => {
		vi.mocked(config.loadConfig).mockResolvedValue({});
		vi.mocked(discover.discoverFiles).mockRejectedValue(
