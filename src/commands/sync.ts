...
export async function preparePlans(
    res: {
        sourcePath: string;
        localeFiles: { locale: string; filePath: string }[];
    },
    opts: SyncOptions,
): Promise<SyncPlan[]> {
...
export function registerSyncCommand(program: Command) {
    program
        ...
        .option(
            "--fail-on-missing",
            "Fail if missing targets exist (no files written)",
            false,
        )
        .option(
            "--graveyard-file <path>",
        ...
        .action(async (opts, cmd) => {
            ...
            const finalOpts: SyncOptions = {
                ...
                dryRun:
                    cmd.getOptionValueSource("dryRun") === "cli"
                        ? opts.dryRun
                        : (syncConfig.dryRun ?? opts.dryRun),
                newTarget:
                    cmd.getOptionValueSource("newTarget") === "cli"
                        ? opts.newTarget
                        : (syncConfig.newTarget ?? opts.newTarget),
                obsolete:
                    cmd.getOptionValueSource("obsolete") === "cli"
                        ? opts.obsolete
                        : (syncConfig.obsolete ?? opts.obsolete),
                failOnMissing:
                    cmd.getOptionValueSource("failOnMissing") === "cli"
                        ? opts.failOnMissing
                        : (syncConfig.failOnMissing ?? opts.failOnMissing),
                graveyardFile:
                    cmd.getOptionValueSource("graveyardFile") === "cli"
                        ? opts.graveyardFile
                        : (syncConfig.graveyardFile ?? opts.graveyardFile),
            };
            ...
            try {
                ...
                if (finalOpts.dryRun) {
                    const totalAdded = plans.reduce((acc, p) => acc + p.stats.added, 0);
                    const totalObsolete = plans.reduce((acc, p) => acc + p.stats.obsolete, 0);
                    ui.success(
                        `Dry-run complete. Would add ${totalAdded} keys and mark/delete ${totalObsolete} obsolete keys across ${plans.length} files.`,
                    );
                } else {
                    ui.success(
                        finalOpts.obsolete === "graveyard"
                            ? "Sync OK (graveyard)"
                            : "Sync OK",
                    );
                }
