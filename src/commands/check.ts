...
import type { SummaryRow } from "../ui/table.js";
...
export function registerCheckCommand(program: Command) {
	program
		...
		.option(
			"--fail-on-missing",
			"Exit non-zero if missing targets exist",
			false,
		)
		.option("--fail-on-obsolete", "Exit non-zero if obsolete keys exist", false)
		.option(
			"--fail-on-added",
			"Exit non-zero if new keys would be added",
			false,
		)
		.option(
			"--new-target <mode>",
			"todo | empty | source (used for diff only)",
			"todo",
		)
			...
			const finalOpts: CheckOptions = {
				...
				failOnMissing:
					cmd.getOptionValueSource("failOnMissing") === "cli"
						? opts.failOnMissing
						: (checkConfig.failOnMissing ?? opts.failOnMissing),
				failOnObsolete:
					cmd.getOptionValueSource("failOnObsolete") === "cli"
						? opts.failOnObsolete
						: (checkConfig.failOnObsolete ?? opts.failOnObsolete),
				failOnAdded:
					cmd.getOptionValueSource("failOnAdded") === "cli"
						? opts.failOnAdded
						: (checkConfig.failOnAdded ?? opts.failOnAdded),
				newTarget:
					cmd.getOptionValueSource("newTarget") === "cli"
						? opts.newTarget
						: (checkConfig.newTarget ?? opts.newTarget),
				verbose:
					cmd.getOptionValueSource("verbose") === "cli"
						? opts.verbose
						: (checkConfig.verbose ?? opts.verbose),
			};
